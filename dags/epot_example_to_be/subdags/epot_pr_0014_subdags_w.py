...
nr_cell_traffic_ss_prb = [
    {
        ...
        'HadoopJarStep': {
            ...
            'Args': ['spark-submit', '--deploy-mode', 'cluster',
                    f's3://{PROJECT_BUCKET}/scripts/spark/nr_cell_traffic_ss_prb.jar',
                    DATALAKE_BUCKET, ODS_DB, DWH_DB, CURRENT_YEAR, CURRENT_MONTH]
        },
    }
]

exp_access_pms_nr_wcell = [
    {
        ...
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': ['spark-submit', '--deploy-mode', 'cluster',
                    f's3://{PROJECT_BUCKET}/scripts/spark/exp_access_pms_nr_wcell.jar',
                    DATALAKE_BUCKET, ODS_DB, DWH_DB, CURRENT_YEAR, CURRENT_MONTH]
        },
    }
]
...
spark_dic = {
    'HETL-0009': nr_cell_traffic_ss_prb,
    'ETL-0013': exp_access_pms_nr_wcell
}
...
def create_epot_subdag_w(parent_dag_name, child_dag_name, default_args):
    with DAG(
