...
exp_access_pms_nr_dcell = [
    {
        ...
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': ['spark-submit', '--deploy-mode', 'cluster',
                    f's3://{PROJECT_BUCKET}/scripts/spark/exp_access_pms_nr_dcell.jar',
                    DATALAKE_BUCKET, ODS_DB, DWH_DB, CURRENT_YEAR, CURRENT_MONTH]
        },
    }
]

procedure_list = ['HETL-0007','ETL-0011']

spark_dic = {
    'HETL-0007': nr_dcell_traffic_ss_prb,
    'ETL-0011': exp_access_pms_nr_dcell
}
...
def create_epot_subdag_d(parent_dag_name, child_dag_name, default_args):
    with DAG(
