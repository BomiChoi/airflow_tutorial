...
nr_ss_cell_info = [
    {
        'Name': 'Invoice Fact Table Processing Step 1',
        'ActionOnFailure': 'CONTINUE',
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': ['spark-submit', '--deploy-mode', 'cluster',
                    f's3://{PROJECT_BUCKET}/scripts/spark/nr_ss_cell_info.jar',
...
nr_hcell_traffic = [
    {
        ...
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': ['spark-submit', '--deploy-mode', 'cluster',
                    f's3://{PROJECT_BUCKET}/scripts/spark/nr_hcell_traffic.jar',
                    DATALAKE_BUCKET, ODS_DB, DWH_DB, CURRENT_YEAR, CURRENT_MONTH]
        },
    }
]

exp_access_pms_nr_hcell = [
    {
        ...
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': ['spark-submit', '--deploy-mode', 'cluster',
                    f's3://{PROJECT_BUCKET}/scripts/spark/exp_access_pms_nr_hcell.jar',
                    DATALAKE_BUCKET, ODS_DB, DWH_DB, CURRENT_YEAR, CURRENT_MONTH]
        },
    }
]

procedure_list = ['HETL-0005', 'HETL-0006','ETL-0010']

spark_dic = {
    'HETL-0005': nr_ss_cell_info,
    'HETL-0006': nr_hcell_traffic,
    'ETL-0010': exp_access_pms_nr_hcell
}
...
def create_epot_subdag_h(parent_dag_name, child_dag_name, default_args):
    with DAG(
